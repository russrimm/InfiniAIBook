...
OpenNotebook is a single-user, self-hosted application. Keep the following in
mind before running it anywhere other than your own machine:

- **There is no authentication.** Anyone who can reach the server can read every
  notebook, add sources, and spend your model, Speech and search quota using the
  credentials the server runs with. Bind it to localhost
  (`npm run dev -- -H 127.0.0.1`, `npm start -- -H 127.0.0.1`) or put it behind
